    ...
    def __init__(self, sps: float = 1, csv: bool = False, print: bool = True) -> None: 
        ...
        if sps != 0:
            ...
            if (self.print or self.csv):
                threading.Thread(target = self.sample()) # We start the sampling thread
...
def sample(self, print: bool, csv: bool):
    t_ini = time()
    if self.state == 2: #So that we only take measurements when the machine is running a test    
        measures = self.get_measurements()
        if print: log.info(measures) #TODO: it might be necessary to parse the dictionary into string or something
        if csv: self.measures.update(measures)  
